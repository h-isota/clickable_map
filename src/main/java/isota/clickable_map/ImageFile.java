...
public class ImageFile {
	...
	for (Shape s : shapes) {
	    ...
	    if (sa.getX() + sa.getWidth() > x2) {
		x2 = sa.getX() + sa.getWidth();
	    }
	    if (sa.getY() < y1) {
		y1 = sa.getY();
	    }
	    if (sa.getY() + sa.getHeight() > y2) {
		y2 = sa.getY() + sa.getHeight();
	    }
	}
	...
	final int MARGIN = 10;
	Area a = getArea();
	BufferedImage bi = new BufferedImage(a.getX() + a.getWidth() + MARGIN, a.getY() + a.getHeight() + MARGIN,
	...
	sb.append(String.format("<img src=\"%s\" alt=\"%s\" usemap=\"#%s\">", src, name, name))
		.append(Utils.getLineCd());
    ...
    /**
     * クリッカブルマップタグを取得します。 {@link #save(File)} で保存した後に呼び出す必要があります。
